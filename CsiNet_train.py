...
def residual_network(x, residual_num, encoded_dim):
    ...
    def complex_BN(xr, xi, name='BN'):
        with tf.variable_scope(name):
            ...
            gamma_ii = tf.get_variable(name='gamma_ii',initializer=tf.convert_to_tensor(1/np.sqrt(2)))
            gamma_ri = tf.get_variable(name='gamma_ri',initializer=tf.convert_to_tensor(0))
